...
/** @defgroup DriverIim423xxDriver_HL_apex Iim423xx driver high level functions related to APEX
 *  @brief High-level function to setup an Iim423xx device
 *  @ingroup  DriverIim423xx
 *  @{
 */

/** @file Iim423xxDriver_HL_apex.h
 * High-level function to setup an Iim423xx device
 */

#ifndef _INV_ICM4235x_DRIVER_HL_APEX_H_
#define _INV_ICM4235x_DRIVER_HL_APEX_H_
...
/** @brief Iim423xx TAP inputs parameters definition
 */
typedef struct {
	uint8_t min_jerk_thr;	                                /**< Minimum Jerk Threshold */
	IIM423XX_APEX_CONFIG7_TAP_MAX_PEAK_TOL_t max_peak_tol;  /**< Maximum peak tolerance */
	IIM423XX_APEX_CONFIG8_TAP_TMAX_t tmax;                  /**< Tap measurement window */
	IIM423XX_APEX_CONFIG8_TAP_TAVG_t tavg;                  /**< Energy measumerement window */
	IIM423XX_APEX_CONFIG8_TAP_TMIN_t tmin;                  /**< Single tap window */
} inv_iim423xx_tap_parameters_t;

/** @brief Iim423xx APEX inputs parameters definition
 */
typedef struct inv_iim423xx_apex_parameters {
	IIM423XX_APEX_CONFIG2_PEDO_AMP_TH_t pedo_amp_th;                   /**< Peak threshold value to be considered as a valid step (mg) */
	...
	uint8_t pedo_step_det_th;                                          /**< Minimum number of low latency steps that must be detected 
	                                                                        before the pedometer step count begins incrementing */
	IIM423XX_APEX_CONFIG3_PEDO_SB_TIMER_TH_t pedo_sb_timer_th;         /**< Duration of non-walk to exit the current walk mode, 
	                                                                        pedo_step_cnt_th number of steps must again be detected 
	                                                                        before step count starts to increase */
	IIM423XX_APEX_CONFIG3_PEDO_HI_ENRGY_TH_t pedo_hi_enrgy_th;         /**< Threshold to improve run detection if not steps are counted while running */
 
	IIM423XX_APEX_CONFIG4_TILT_WAIT_TIME_t tilt_wait_time;             /**< Number of accelerometer samples to wait before triggering tilt event */
 
	IIM423XX_APEX_CONFIG1_DMP_POWER_SAVE_TIME_t power_save_time;       /**< The time after which DMP goes in power save mode according to the DMP ODR configured */
	IIM423XX_APEX_CONFIG0_DMP_POWER_SAVE_t power_save;                 /**< Power save mode for APEX algorithms. This mode will put APEX features into sleep mode, 
	                                                                        leaving only the WOM running to wake-up the DMP */
	
	IIM423XX_APEX_CONFIG9_SENSITIVITY_MODE_t sensitivity_mode;         /**< Sensitivity mode Normal(0) or Slow walk(1). The Slow walk mode improve 
	                                                                        the slow walk detection (<1Hz) but in return the number of false detection 
	                                                                        might be increase. */

#if defined(ICM_FAMILY_BPLUS)

	IIM423XX_APEX_CONFIG4_R2W_SLEEP_TIME_OUT_t r2w_sleep_time_out;     /**< Time out for a sleep detection. */
	IIM423XX_APEX_CONFIG5_R2W_MOUNTING_MATRIX_t r2w_mounting_matrix;   /**< Mounting matrix, chip to device frame. */
	IIM423XX_APEX_CONFIG6_R2W_SLEEP_GEST_DELAY_t r2w_gest_delay;       /**< Detection window for a sleep gesture detection. */

#elif defined(ICM_FAMILY_CPLUS)

	IIM423XX_APEX_CONFIG10_FF_DEBOUNCE_DURATION_t ff_debounce_duration;      /**< Duration(us) during which LowG and HighG events are not taken into account after an HighG event.
	                                                                             The goal is to avoid detecting bounces as free falls */
	IIM423XX_APEX_CONFIG10_FF_MAX_DURATION_t ff_max_duration_cm;            /**< Distance (cm) max crossed by the device after a LowG event, below which the detection of an HighG event triggers a freefall interrupt */
	IIM423XX_APEX_CONFIG10_FF_MIN_DURATION_t ff_min_duration_cm;            /**< Distance (cm) min crossed by the device after a LowG event, above which the detection of an HighG event triggers a freefall interrupt */
	
	IIM423XX_APEX_CONFIG5_LOWG_PEAK_TH_t lowg_peak_th;                     /**< Absolute low peak threshold (mg) to detect when it falls below on any of the accelerometer axis */
	IIM423XX_APEX_CONFIG4_LOWG_PEAK_TH_HYST_t lowg_peak_hyst;              /**< Hysteresis threshold (mg) added to the threshold after the initial threshold is met */
	IIM423XX_APEX_CONFIG5_LOWG_TIME_TH_SAMPLES_t lowg_samples_th;          /**< Time in number of samples to stay below the threshold before triggering the event (samples) */
	
	IIM423XX_APEX_CONFIG6_HIGHG_PEAK_TH_t highg_peak_th;                   /**< Absolute low peak threshold (mg) to detect when it falls below on any of the accelerometer axis */
	IIM423XX_APEX_CONFIG4_HIGHG_PEAK_TH_HYST_t highg_peak_hyst;            /**< Hysteresis threshold (mg) added to the threshold after the initial threshold is met */
	IIM423XX_APEX_CONFIG6_HIGHG_TIME_TH_SAMPLES_t highg_samples_th;        /**< Time in number of samples to stay below the threshold before triggering the event (samples) */

#endif

	IIM423XX_APEX_CONFIG1_LOW_ENERGY_AMP_TH_t low_energy_amp_th;       /**< Peak threshold value to be considered as a valid step (mg) in Slow walk mode */
} inv_iim423xx_apex_parameters_t;

/** @brief APEX pedometer outputs
 */
typedef struct inv_iim423xx_apex_step_activity {
...
} inv_iim423xx_apex_step_activity_t;

/** @brief TAP outputs
 */
typedef struct inv_iim423xx_tap_data {
	IIM423XX_APEX_DATA4_TAP_NUM_t tap_num;     /**< Detects single (1) or double (2) tap*/
	IIM423XX_APEX_DATA4_TAP_AXIS_t tap_axis;   /**< Axis along which tap has been detected */
	IIM423XX_APEX_DATA4_TAP_DIR_t tap_dir;     /**< Direction of the tap, either +axis (1) or -axis (0)*/
 ...
 *  @param[out] tap_inputs Default input parameters. See @sa inv_iim423xx_tap_parameters_t 
 *  @return 0 on success, negative value on error.
 */
int inv_iim423xx_init_tap_parameters_struct(struct inv_iim423xx *s, inv_iim423xx_tap_parameters_t *tap_inputs);

/** @brief  Configure TAP.
 *  @param[in] tap_inputs The requested input parameters. See @sa inv_iim423xx_tap_parameters_t
 *  @return 0 on success, negative value on error.
 */
 ...
 *  @param[out] tap_params The current parameter, fetched from registers. See @sa inv_iim423xx_tap_parameters_t
 *  @return 0 on success, negative value on error.
 */
 ...
 *  @param[out] apex_inputs Default input parameters. See @sa inv_iim423xx_apex_parameters_t 
 *  @return 0 on success, negative value on error.
 */
 ...
 *  @param[in] apex_inputs The requested input parameters. See @sa inv_iim423xx_apex_parameters_t
 ...
 *  @param[out] apex_params The current parameter, fetched from registers. See @sa inv_iim423xx_apex_parameters_t
 *  @return 0 on success, negative value on error.
 */
 ...
 *  @sa IIM423XX_APEX_CONFIG0_DMP_ODR_t
 ...
 */
int inv_iim423xx_load_dmp_sram_code(struct inv_iim423xx *s, const uint8_t *dmp_prog, const uint32_t start_offset, const uint32_t size);

#ifdef __cplusplus
}
#endif

#endif /* _INV_ICM4235x_DRIVER_HL_APEX_H_ */
