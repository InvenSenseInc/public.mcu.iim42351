...
int SetupInvDevice(struct inv_iim423xx_serif * icm_serif)
{
	...
	rc = inv_iim423xx_init(&icm_driver, icm_serif, HandleInvDeviceDataRegisters);
		...
		uint8_t is_rtc_mode)					   
{
	...
	/* Wait IIM423XX_ACC_STARTUP_TIME_US*/
	inv_iim423xx_sleep_us(IIM423XX_ACC_STARTUP_TIME_US);
...
int GetDataFromInvDevice(void)
{
	 ...
	 * FIFO.
	 */
	return inv_iim423xx_get_data_from_registers(&icm_driver);
}

void HandleInvDeviceDataRegisters(inv_iim423xx_sensor_event_t * event)
{
