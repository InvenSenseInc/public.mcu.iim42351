...
/* Function definition */
static uint8_t * get_register_cache_addr(struct inv_iim423xx * s, uint8_t reg);
static uint8_t is_aux_interface(struct inv_iim423xx_transport *t);
...
int inv_iim423xx_init_transport(struct inv_iim423xx * s)
{
	...
	int status = 0;
	struct inv_iim423xx_transport *t = (struct inv_iim423xx_transport *)s;
...
{
	// First field of struct inv_iim423xx is assumed to be a struct inv_iim423xx_transport object.
	// So let's cast s to struct inv_iim423xx_transport and ignore the rest of struct inv_iim423xx.
	struct inv_iim423xx_transport *t = (struct inv_iim423xx_transport *)s;
	uint32_t i=0;
...
static uint8_t is_aux_interface(struct inv_iim423xx_transport *t)
{
	if ((t->serif.serif_type == IIM423XX_AUX1_SPI3) || (t->serif.serif_type == IIM423XX_AUX2_SPI3) || (t->serif.serif_type == IIM423XX_AUX1_SPI4))
		return 1;
	else
		return 0;
}
